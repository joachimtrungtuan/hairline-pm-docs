...
|-------|-----------|---------------|-----------|-----------------|--------|---------------|----------|-------|
| P-AUTH-001 | Provider owner can log in | Seeded provider1 | `provider1@hairline.app` / `password123` | Login successful toast, redirect to `/`, provider name in header | | | | |
...
| P-AUTH-006 | Session persists on page refresh | Logged in | Refresh browser | Still logged in, dashboard visible | | | | |
| P-AUTH-007 | Logout clears session | Logged in | Click user menu → Logout | Redirected to `/auth`, localStorage cleared | | | | |
...
| TC ID | Test Case | Preconditions | Test Data | Expected Result | Status | Actual Result | Severity | Notes |
|-------|-----------|---------------|-----------|-----------------|--------|---------------|----------|-------|
| P-AUTH-013 | Login with wrong password | Seeded provider | Correct email, wrong password | Error message, stay on login page | | | | |
...
| P-AUTH-017 | Login with both fields empty | On login page | Both empty | Validation errors on both fields | | | | |
| P-AUTH-018 | Provider cannot access admin routes | Logged in as provider | Navigate to admin-only URL | Redirected or access denied | | | | |
| P-AUTH-019 | Unauthenticated user redirected to login | Not logged in | Navigate to `/` (dashboard) | Redirected to `/auth` | | | | |
| P-AUTH-020 | Expired token requires re-authentication | Logged in with expired token | Make API request | Protected data not returned; user must authenticate again before continuing | | | | |
| P-AUTH-021 | API rejects request with no auth token | Not logged in | `GET /api/provider/inquiries` without token | 401 Unauthorized | | | | |
...
| TC ID | Test Case | Preconditions | Test Data | Expected Result | Status | Actual Result | Severity | Notes |
|-------|-----------|---------------|-----------|-----------------|--------|---------------|----------|-------|
| P-INQ-001 | Inquiry list loads | Logged in, inquiries seeded | Navigate to Inquiries | List of inquiry cards displayed | | | | |
...
| P-INQ-015 | Empty inquiry list | Provider with no inquiries | Navigate to Inquiries | Empty state message displayed | | | | |
...
| P-INQ-022 | Provider only sees inquiries assigned to them | Multiple providers | Login as provider1, check list | Only inquiries distributed to this provider shown | | | | |
...
| P-XCT-010 | API enforces role permissions | Token of Billing Staff role | `PUT /api/provider/settings` (owner-only) | 403 Forbidden | | | | |
...
|-------|-----------|-----------------|--------|---------------|----------|-------|
| P-SMK-001 | Provider login succeeds | Dashboard loads | | | | |
| P-SMK-002 | Inquiry list page loads | Inquiries displayed (or empty state) | | | | |
| P-SMK-003 | Quote creation form opens | Form renders with fields | | | | |
| P-SMK-004 | Appointments page loads | Appointments list displayed | | | | |
| P-SMK-005 | Treatment detail page loads | Treatment data rendered | | | | |
| P-SMK-006 | Aftercare page loads | Aftercare section rendered | | | | |
| P-SMK-007 | Settings page loads | Settings with sections rendered | | | | |
| P-SMK-008 | Notifications dropdown opens | Dropdown appears with items or empty state | | | | |
| P-SMK-009 | API health check | `GET /api/` returns response (not 500) | | | | |
| P-SMK-010 | Provider API auth works | `GET /api/provider/inquiries` with token returns 200 | | | | |
